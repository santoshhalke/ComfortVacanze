  ...
  const destinations = [
    {
      ...
      description: 'A tropical escape known for its lush jungles, serene temples, and vibrant beach life.',
    },
    {
      id: 2,
      name: 'Krabi, Thailand',
      image: '/images/Home/krabi.jpg',
      description: 'Home to stunning limestone cliffs, turquoise waters, and peaceful island vibes.',
    },
    {
      id: 3,
      name: 'Sri Lanka',
      image: '/images/Home/beruwalaSrilanka.jpg',
      description: 'A cultural gem filled with ancient ruins, scenic tea hills, and exotic wildlife adventures.',
    },
    {
      id: 4,
      name: 'Vietnam',
      image: '/images/Home/vietnam.jpg',
      description: 'A land of contrast with bustling cities, quiet rice fields, and rich historical charm.',
    },
    {
      id: 5,
      name: 'Phuket, Thailand',
      image: '/images/Home/phuket.jpg',
      description: 'Thailand’s largest island offering golden beaches, lively nightlife, and luxury retreats.',
    },
  ];
        ...
        >
          <div className="flex space-x-6 pb-6">
            {destinations.map((destination) => (
              ...
              >
                <motion.div
                  className={`bg-white rounded-4xl shadow-md overflow-hidden cursor-pointer h-full ${expandedCard === destination.id ? 'ring-2 ring-orange-500' : ''}`}
                ...
                className={`absolute -top-1 -right-3 bg-orange-500 text-white rounded-full w-8 h-8 flex items-center justify-center shadow-lg ${expandedCard === destination.id ? 'z-10' : ''}`}
              >
        ...
        <div className="flex justify-center mt-6 space-x-2">
          {Array.from({ length: destinations.length - 3 }).map((_, index) => (
            <button
              ...
              className={`w-3 h-3 rounded-full transition-colors ${scrollPosition === index ? 'bg-orange-500' : 'bg-gray-300'}`}
